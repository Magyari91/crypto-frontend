...
function chartDate(value) {
  return new Intl.DateTimeFormat("hu-HU", { month: "short", day: "numeric" }).format(
    new Date(value)
  );
}
...
function ChartTooltip({ active, payload }) {
  ...
  return (
    <div className="chart-tooltip">
      <span>{chartDate(point.timestamp)}</span>
  ...
  return (
    <section className="surface chart-panel" aria-labelledby="chart-title">
      <header className="panel-heading">
        <div>
          <span>Árfolyamgörbe</span>
          <h2 id="chart-title">{selected.symbol} piaci trend</h2>
        </div>
        <div className="chart-legend">
          <span><i className="actual-line" />Tényadat</span>
          <span><i className="target-line" />Modell cél</span>
        </div>
      </header>

      <div className="chart-area" aria-label={`${selected.name} árfolyamdiagram`}>
